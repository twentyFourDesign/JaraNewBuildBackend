...
const { lookoutModel } = require("../models");
const { statusCode } = require("../utils/statusCode");


const create = asyncErrorHandler(async(req,res)=>{
    if(req.body.id){
        let update = await lookoutModel.findByIdAndUpdate(req.body.id,req.body)
        if(update){
            res.status(statusCode.accepted).json(update)
        }
    }
    else{
        let create = await lookoutModel.create(req.body)
        if(create){
...
const getAll = asyncErrorHandler(async(req,res)=>{
    let allDaypass = await lookoutModel.find({})
...
const del = asyncErrorHandler(async(req,res)=>{
    let allDaypass = await lookoutModel.findByIdAndDelete(req.param.id)
